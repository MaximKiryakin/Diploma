...
## 🚀 Новые возможности: Активное управление портфелем

### Обновление v2.0 - Добавлено управление портфелем

Проект теперь включает полноценные инструменты активного управления:

- **Оптимизация весов портфеля** (минимальный риск, максимальный Шарп, risk parity)  
- **Автоматическая ребалансировка** с настраиваемыми порогами
- **Портфельный анализ рисков** с учетом диверсификации
- **Визуализация оптимальных весов** и торговых сигналов
...
calc = (
    ...
    .optimize_portfolio_weights(                # Оптимизация по Шарпу
        method='max_sharpe', risk_free_rate=0.08
    )
    .plot_optimal_weights(verbose=False)        # Визуализация весов  
    .calculate_risk_adjusted_pd()               # Портфельный PD
    .rebalance_portfolio(                       # Ребалансировка
...
print(f"Коэффициент Шарпа: {summary['sharpe_ratio']:.3f}")
print(f"Портфельный PD: {summary['adjusted_portfolio_pd']:.4f}")
```

### Быстрый старт:
```bash
# Демонстрация всех возможностей управления портфелем
python portfolio_management_example.py
```

Подробное руководство: [PORTFOLIO_MANAGEMENT.md](PORTFOLIO_MANAGEMENT.md)
