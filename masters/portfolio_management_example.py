...
def main():
    ...
    try:
        ...
        calc_sharpe = Portfolio(
            dt_calc='2025-05-31', dt_start='2019-11-03', 
            stocks_step=10, tickers_list=tickers_list
        )
        ...
        print(f"  Ожидаемая доходность: {summary_min_var.get('expected_return', 'N/A'):.4f}")
        print(f"  Волатильность: {summary_min_var.get('volatility', 'N/A'):.4f}")
        print(f"  Коэффициент Шарпа: {summary_min_var.get('sharpe_ratio', 'N/A'):.4f}")
        
        print(f"\nМаксимальный Шарп:")
        print(f"  Ожидаемая доходность: {summary_max_sharpe.get('expected_return', 'N/A'):.4f}")
        print(f"  Волатильность: {summary_max_sharpe.get('volatility', 'N/A'):.4f}")
        print(f"  Коэффициент Шарпа: {summary_max_sharpe.get('sharpe_ratio', 'N/A'):.4f}")
        ...
        if hasattr(calc, 'rebalancing_trades') and calc.rebalancing_trades:
            ...
            for ticker, trade_info in calc.rebalancing_trades.items():
                ...
                print(f"  {ticker}: {action} {size:.3f} ({trade_info['current_weight']:.3f} → {trade_info['target_weight']:.3f})")
        else:
